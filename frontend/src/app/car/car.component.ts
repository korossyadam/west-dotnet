...
export class CarComponent implements OnInit {
   ...
   private _transformer = (node: CategoryNode, level: number) => {
      return {
         expandable: (!!node.children && node.children.length > 0) || (!node.leaf && node.amount && node.amount > 0),
         name: node.name,
         value: node.value,
         amount: node.amount,
         level: level,
   ...
   async getProducts(node: FlatNode) {
      ...
      if(node.level == 0) {
        ...
        /*
         this.carService.getProducts(this.currentCar.carIndex, node.name).subscribe(products => {
            ...
            for(let i = 0; i < this.originalTreeData.length; i++) {
               ...
               if(result != -1) {
                  ...
                  for(let j = 0; j < products.length; j++) {
                     ...
                     for(let k = 2; k < products[j].categories.length; k++) {

                        // If not already exists, append category to tree
                        let element = obj.children.filter(e => e.name == products[j].categories[k]);
                        if(element.length <= 0) {
                           obj.children.push({ name: products[j].categories[k], children: [], amount: 1 });
                        } else {
                           element[0].amount += 1;
                        }

                        // Move down pointer within tree by one
                        let idx = obj.children.findIndex(e => e.name == products[j].categories[k]);
                        obj = obj.children[idx];
                     }

                     // Save already queried products in sessionStorage to later be used in products component
                     this.saveInSessionStorage(products, node);
